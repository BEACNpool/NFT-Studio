...
for (const mode of ['auto', 'legacy']) {
  ...
  try {
    ...
    const expectedNames=PUBLIC_TOOL_NAMES.filter(name=>(expectedTools>=26||!['studio_workbench','get_utility_recipe','plan_nft_utility','inspect_mint_readiness'].includes(name))&&(expectedTools>=22||!['studio_utilities','configure_mint_options','create_payload_qr'].includes(name))&&(expectedTools>=19||!['create_mobile_handoff','revoke_mobile_handoff'].includes(name))&&(expectedTools>=17||!['studio_guide','studio_inspiration'].includes(name))&&(expectedTools>=9||name!=='prepare_unsigned_transaction')&&(expectedTools>=10||name!=='apply_state_capsule_parameters')&&(expectedTools>=12||!['create_music_release','verify_music_release'].includes(name))&&(expectedTools>=13||name!=='prepare_unsigned_music_transaction')&&(expectedTools>=15||!['search_cip_sources','get_cip_source_chunk'].includes(name)));
