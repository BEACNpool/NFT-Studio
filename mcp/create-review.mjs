...
export async function readRequest(requestPath){
 ...
 fields(value,['mode','name','files'],['description','coverIndex']);if(!['nft','data'].includes(value.mode))throw Error('Mode must be nft or data.');boundedText(value.name,64);if(value.description!==undefined)boundedText(value.description,1024,true);
 ...
 return {mode:value.mode,name:value.name,...(value.description===undefined?{}:{description:value.description}),...(value.coverIndex===undefined?{}:{coverIndex:value.coverIndex}),files};
}
...
export function verifyHandoff(made,verified,args){
 ...
 fields(intent,['schema','mode','bundle','intentHash']);assert.equal(intent.schema,'nft-studio.intent.v1');assert.equal(intent.mode,args.mode);const b=intent.bundle;fields(b,['schema','name','description','cover','bytes','files','sha256']);assert.equal(b.schema,'nft-studio.payload.v1');assert.equal(b.name,args.name);assert.equal(b.description,args.description||'');assert.equal(b.cover,args.coverIndex!==undefined);
