...
test('Modern and legacy official SDK clients discover tools and read fixed resources across repeated stateless HTTP calls',async()=>{
  for(const era of ['auto','legacy']) {
    ...
    try{
      ...
      const resource=await ctx.client.readResource({uri:'nft-studio://capabilities'});assert.equal(JSON.parse(resource.contents[0].text).serverVersion,'0.4.0');
...
test('Actual stdio child process supports both protocol eras with protocol-only stdout',async()=>{
  for(const era of ['auto','legacy']){
    ...
    try{await client.connect(transport);assert.equal(client.getProtocolEra(),era==='auto'?'modern':'legacy');assert.deepEqual((await client.listTools()).tools.map(t=>t.name).sort(),NODE_TOOL_NAMES);assert.equal((await call(client,'studio_capabilities')).serverVersion,'0.4.0');assert.equal(errors,'');}
