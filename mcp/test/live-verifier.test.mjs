...
test('Seventeen-tool release verifier passes modern/legacy real Workerd with only local synthetic reads',async()=>{
  const handler=await createPublicMcpHandler({publicOrigin:origin,endpointPath:'/api/mcp'});
  try{
    const result=await verifyLiveEndpoint({endpoint,expectedTools:17,expectedResources:resourceCount,fetchImpl:(url,init)=>handler.dispatchFetch(url,init)});
