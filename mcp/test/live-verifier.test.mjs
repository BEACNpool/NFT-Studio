...
async function eightFixture({missingCors=false,hideUnsigned=true,keepCapsule=false,keepMusic=false,keepImplementations=false,keepUnsignedMusic=false}={}){
  ...
  const fetchImpl=async(url,init)=>{
    ...
    const replace=text=>{const body=JSON.parse(text);if(request.method==='tools/list'){assert.ok(Array.isArray(body.result.tools));body.result.tools=body.result.tools.filter(t=>!['studio_workbench','get_utility_recipe','plan_nft_utility','inspect_mint_readiness','studio_utilities','configure_mint_options','create_payload_qr','create_mobile_handoff','revoke_mobile_handoff','studio_guide','studio_inspiration','search_cip_sources','get_cip_source_chunk',...(!keepUnsignedMusic?['prepare_unsigned_music_transaction']:[])].includes(t.name));if(!keepMusic)body.result.tools=body.result.tools.filter(t=>!['create_music_release','verify_music_release'].includes(t.name));if(!keepCapsule)body.result.tools=body.result.tools.filter(t=>t.name!=='apply_state_capsule_parameters');if(hideUnsigned)body.result.tools=body.result.tools.filter(t=>t.name!=='prepare_unsigned_transaction');}else{assert.ok(Array.isArray(body.result.resources));body.result.resources=body.result.resources.filter(r=>r.uri!=='ui://nft-studio/workbench/v1');if(!keepImplementations)body.result.resources=body.result.resources.filter(r=>r.uri!=='nft-studio://implementations').slice(0,55);}return JSON.stringify(body);};
