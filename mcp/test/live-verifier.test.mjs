...
test('Twenty-two-tool release verifier passes modern/legacy real Workerd with only local synthetic reads',async()=>{
  const handler=await createPublicMcpHandler({publicOrigin:origin,endpointPath:'/api/mcp'});
  try{
    const result=await verifyLiveEndpoint({endpoint,expectedTools:22,expectedResources:resourceCount,fetchImpl:(url,init)=>handler.dispatchFetch(url,init)});
