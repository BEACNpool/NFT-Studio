...
const jsonResult = output => ({content:[{type:'text',text:JSON.stringify(output)}],structuredContent:output});
  ...
  function factory() {
    ...
    register('prepare_unsigned_transaction','Build an actual unsigned mainnet NFT or data transaction with the shared Studio builder and live bounded protocol feed. Caller supplies CIP-30 change address and UTxO CBOR, kept in RAM for four minutes. Inputs are caller assertions; chain unspent state is not independently verified. All outputs return to that wallet. Does not sign or submit.',prepareSchema,async ({intent,wallet})=>preparationGate.run(async ()=>{
      ...
      try {
        ...
        const prepared=await buildStudioTransaction(C,verified.bundle,verified.mode,w,p);
        ...
        return {
          ...
          asset:prepared.mode==='nft'?{policyId:prepared.policyId,assetName:prepared.assetName,assetNameHex:Buffer.from(prepared.assetName).toString('hex'),quantity:'1',policyScriptHex:prepared.policyScript,policyExpirySlot:prepared.expirySlot}:null,
          ...
          protocol:prepared.protocol,metadata:prepared.metadata,policySemantics:CAPABILITIES.policy,
