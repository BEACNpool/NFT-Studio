...
The dedicated music-package tools retain their existing file-import handoff. This helper is for ordinary NFT/data payload intents only. Cross-platform paths and a shell-free Node child-process invocation are used; actual tests in this kit ran on Linux, not Windows.
