...
For MCP, add one fixed read-only resource, `nft-studio://implementations`, backed by the validated bundled JSON. The existing `read_knowledge` response can optionally include related implementation IDs without changing its research entry. Avoid adding caller-supplied URL/path reads, tools that promote status, or live chain claims. The optional resource is an integration proposal in this kit; it is not registered by the helper. Adding it would change the current resource count from 55 to 56 and requires a new live verification receipt. Preserve older receipts as dated observations.
