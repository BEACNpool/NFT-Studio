...
The first unsigned-builder receipt observed nine tools and 55 resources at 2026-09-07T06:28:25.565Z, including independently checked synthetic NFT/data unsigned output. The earlier eight-tool observation is retained. These are dated observations of the deployed public service; neither establishes chain availability for fabricated inputs or signing/submission authority. The separately installed Node witness-verification service is not claimed as remotely deployed. The source-drift sample similarly describes one upstream comparison, not perpetual freshness or a successful scheduled workflow run.

The Capsule parameterizer now has separate local evidence for 256 Aiken parameter pairs, independent CSL script hashes and 256 browser oracle cases, plus a developer-observed 15-check integrated desktop/mobile browser run. This is fixed-blueprint parameter application; it does not add a wallet flow or broaden the older independent-node issuance observation.

Music evidence covers the exact-file/credit codec, shared native transaction adapter, canonical package composer, strict signed/unsigned receipt recovery and package-only MCP tools. Its transaction and receipt fixtures are synthetic. A developer-observed local browser run passed seven groups; local and clean-install MCP suites each passed 37 groups with 12 tools in the local Worker, 14 in Node and 61 resources. These are local Node/Workerd/SDK observations; this admission adds no Music live-service evidence or chain inclusion claim. The browser and MCP procedure pins identify the published test source; their observation timestamps come from local reports/log modification times, not authenticated run receipts.
