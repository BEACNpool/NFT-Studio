...
const CACHE = "nft-studio-ledger-shell-v1";
const SHELL = [
  "./",
  "./index.html",
  "./calculator.html",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith("nft-studio-ledger-shell-") && k !== CACHE).map((k) => caches.delete(k)))
